...
class YDMHttp(object):
    ...
    def decode(self, filename, codetype, timeout):
        ...
        ret_data = json.loads(response_data.text)
        #while ret_data["ret"] == 0 and ret_data["text"] == "":
        #    ret_data = json.loads(requests.post(self.apiurl, files=files, data=data).text)
        if ret_data["ret"] == 0:
...
if __name__ == "__main__":
    ...
    appkey = '8eeeff4c215953c27692796a7ec258bf'
    # 图片文件
    filename = settings.IMAGES_STORE+"\captcha.jpg"
    # 验证码类型，# 例：1004表示4位字母数字，不同类型收费不同。请准确填写，否则影响识别率。在此查询所有类型 http://www.yundama.com/price.html
    codetype = 5000
    # 超时时间，秒
    timeout = 60
